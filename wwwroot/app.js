// Full front-end compatible with UP_CONDI_V5 endpoints
(() => {
  "use strict";
  ...
  function setBusy(isBusy, label) {
    ...
    const btns = page === "login"
      ...
      : [
          ...
          app.btnStatsReload, app.btnAssignMaster, app.btnChangeStatus, app.btnExtendDeadline,
  ...
  async function doLogin(isRegister){
    ...
    try{
      ...
      showMessage("ok", "Success", isRegister ? "Registered and logged in." : "Logged in.");
  ...
  function canUseManagerTools(){
    const role = getUserRole();
    return ["Менеджер", "Оператор", "Master", "Мастер", "Operator"].includes(role);
  }
  ...
  function renderEntities(){
    ...
    const req = ENTITIES.find(e => e.name === "Request");
    if (req) app.entitySelect.value = req.name;
  }
  ...
  function pickCardTitle(row, entity){
    const candidates = ["Fio","ClimateTechModel","Message","name","title","model","type"];
  ...
  function buildRequestUrlFromSearch(){
    const q = (app.searchQ?.value || "").trim();
    let url = apiUrl("/Request");
    if (!q) return url;

    const params = new URLSearchParams();
    if (/^\d+$/.test(q)) params.set("requestId", q);
  ...
  async function reload(){
    ...
    try{
      ...
      if (entity.name === "Request") url = buildRequestUrlFromSearch();
      const rows = await http("GET", url, undefined, true);
      const list = Array.isArray(rows) ? rows : (rows || []);
      setViewRows(entity, list);
      log(`GET ${url} OK`);
      if (entity.name === "Request") loadStats().catch(() => {});
    } catch(e){
  ...
  function openDetails(){
    ...
    for (const k of keys){
      ...
      html.push(`<div style="font-weight:900;opacity:.7;">${escapeHtml(k)}</div>`);
  ...
  function inputTypeFor(t){
    ...
    if (["datetime","datetimeoffset","dateonly"].includes(x)) return "date";
    return "text";
  }
  ...
  function hintFor(field){
    ...
    if (["int","long","short","byte","decimal","double","float","number"].includes(t)) return "Введите число.";
    if (t === "bool" || t === "boolean") return "Флажок.";
    if (t === "datetime" || t === "datetimeoffset" || t === "dateonly") return "Выберите дату.";
    if (n.toLowerCase().includes("phone")) return "Введите телефон.";
  ...
  function renderStatsList(container, items){
    ...
    if (list.length === 0){
      container.innerHTML = `<div class="statsrow"><div class="statsname">Нет данных</div><div class="statscount">0</div></div>`;
      return;
    }
    for (const item of list){
      const row = document.createElement("div");
      row.className = "statsrow";
      row.innerHTML = `<div class="statsname">${escapeHtml(item?.name ?? item?.Name ?? "—")}</div><div class="statscount">${escapeHtml(String(item?.count ?? item?.Count ?? 0))}</div>`;
      container.appendChild(row);
    }
  }

  async function loadStats(){
    if (!app.viewStats) return;
    try{
      const url = apiUrl("/Request/stats");
      const res = await http("GET", url, undefined, true);
      app.statTotal.textContent = String(res?.totalCount ?? res?.TotalCount ?? 0);
      app.statCompleted.textContent = String(res?.completedCount ?? res?.CompletedCount ?? 0);
      const avg = res?.averageRepairDays ?? res?.AverageRepairDays ?? 0;
      app.statAvgDays.textContent = `${avg} дн.`;
      renderStatsList(app.statsByStatus, res?.byStatus ?? res?.ByStatus ?? []);
      renderStatsList(app.statsByType, res?.byClimateTechType ?? res?.ByClimateTechType ?? []);
    } catch(e){
      log(`STATS FAIL: ${String(e?.message || e)}`);
    }
  }

  function renderQualityQr(){
    if (app.qualityFormLink) app.qualityFormLink.textContent = QUALITY_FORM_URL;
    if (app.qualityQrImg) {
      app.qualityQrImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(QUALITY_FORM_URL)}`;
    }
  }

  function syncRequestManager(){
    if (!app.requestManagerPanel) return;
    const entity = getSelectedEntity();
    const isRequest = entity?.name === "Request";
    const show = !!state.selectedRowObj && isRequest && canUseManagerTools();
    app.requestManagerPanel.style.display = show ? "" : "none";
    if (app.managerCurrentRequest) app.managerCurrentRequest.textContent = show ? String(state.selectedRowId ?? "—") : "—";
    if (show && app.managerStatus){
      const status = state.selectedRowObj?.RequestStatus ?? state.selectedRowObj?.requestStatus ?? "Новая";
      app.managerStatus.value = status;
    }
  }

  async function assignMaster(){
    if (!state.selectedRowId) return showMessage("warn","No selection","Select a request first.");
    const raw = (app.managerMasterId?.value || "").trim();
    if (!raw) return showMessage("warn","Master ID","Введите ID мастера.");
    try{
      setBusy(true,"Assign master");
      await http("PATCH", apiUrl(`/Request/${state.selectedRowId}/assign-master`), { masterId: Number(raw) }, true);
      showMessage("ok","Done","Master assigned.");
      await reload();
    } catch(e){
      showMessage("err","Assign error", String(e?.message || e), e?._details || "");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function changeStatus(){
    if (!state.selectedRowId) return showMessage("warn","No selection","Select a request first.");
    const status = (app.managerStatus?.value || "").trim();
    if (!status) return showMessage("warn","Status","Выберите статус.");
    try{
      setBusy(true,"Change status");
      await http("PATCH", apiUrl(`/Request/${state.selectedRowId}/change-status`), { requestStatus: status }, true);
      showMessage("ok","Done","Status updated.");
      await reload();
    } catch(e){
      showMessage("err","Status error", String(e?.message || e), e?._details || "");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function extendDeadline(){
    ...
    if (!completionDate) return showMessage("warn","Date","Выберите дату завершения.");
    try{
      setBusy(true,"Extend deadline");
      await http("PATCH", apiUrl(`/Request/${state.selectedRowId}/extend-deadline`), { completionDate, comment }, true);
      showMessage("ok","Done","Deadline extended.");
      await reload();
    } catch(e){
      showMessage("err","Deadline error", String(e?.message || e), e?._details || "");
    } finally {
      setBusy(false);
    }
  }
  ...
  function initAppPage(){
    ...
    app.navStats?.addEventListener("click", async () => { setActiveNav(app.navStats); showView("view_stats"); await loadStats(); });
    ...
    app.searchQ?.addEventListener("input", () => {
      const entity = getSelectedEntity();
      if (entity?.name !== "Request") searchLocal();
    });

    app.entitySelect?.addEventListener("change", () => {
      syncRequestManager();
      reload();
    });

    app.btnStatsReload?.addEventListener("click", loadStats);
    app.btnAssignMaster?.addEventListener("click", assignMaster);
    app.btnChangeStatus?.addEventListener("click", changeStatus);
    app.btnExtendDeadline?.addEventListener("click", extendDeadline);
    ...
    app.btnCopyQualityLink?.addEventListener("click", async () => {
      try{
        ...
        showMessage("ok", "Copied", "Link copied.");
      } catch {
        showMessage("warn", "Clipboard blocked", "Browser denied clipboard access.");
      }
    });
